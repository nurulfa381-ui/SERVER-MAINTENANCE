...
const a=["👨‍💻","👩‍💻","🛡️","⚙️","🧠","🚀"];
app.innerHTML=`<section class="center"><div class="card login"><h1>SERVER HERO™</h1><h2>${t("login")}</h2><label>${t("name")}</label><input id="name"><label>${t("id")}</label><input id="id"><h3>${t("avatar")}</h3><div class="avatars">${a.map((x,i)=>`<button class="${i===0?"active":""}" data-a="${x}" onclick="pick(this)">${x}</button>`).join("")}</div><button class="primary wide" onclick="saveLogin()">${t("enter")}</button><div class="teacher-entry"><span>ATAU</span><button class="teacher-login-btn wide" onclick="window.location.href='teacher.html'">👨‍🏫 MOD GURU</button><small>Rekod markah rasmi KT</small></div></div></section>`;
}
